...
pub const NES_CRUNCH: [u8; 256] = {
    let mut buf = [0u8; 256];
    let mut state: u16 = 1; // non-zero seed
    let mut i = 0;
    while i < 256 {
        // 93-sample short-mode loop
        let mut j = 0;
        while j < 93 && i < 256 {
            // taps at bit0 ^ bit6
            let bit = (state ^ (state >> 6)) & 1;
            state = (state >> 1) | (bit << 14);
            buf[i] = if (state & 1) != 0 { 0xFF } else { 0x00 };
            j += 1;
            i += 1;
        }
    }
